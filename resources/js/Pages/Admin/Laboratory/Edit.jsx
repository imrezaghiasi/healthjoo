...
const Edit = ({auth, errors}) => {
    ...
    const {laboratory_test, patients} = usePage().props

    const {data, setData} = useForm({
        id: laboratory_test.id || '',
        patient_id: laboratory_test.patient_id || "",
        test_type: laboratory_test.test_type || "",
        price: laboratory_test.price || "",
        _method: 'PUT'
    })
    ...
    function handleSubmit(e) {
        e.preventDefault();
        router.post(route("admin.laboratory_tests.update", laboratory_test.id), data);
    }
    ...
    return (
        <Authenticated
            auth={auth}
            errors={errors}
            header={<h2 className="font-semibold text-xl leading-tight">ویرایش آزمایش</h2>}
        >
