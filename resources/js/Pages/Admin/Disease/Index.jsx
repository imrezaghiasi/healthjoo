...
const Index = (props) => {
    ...
    function destroy(e) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.delete(route("admin.diseases.destroy", e.target.id), {
                    onSuccess: () => {
                        Swal.fire({
                                title: 'حذف شد!',
    ...
    function restore(e) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.get(route("admin.diseases.restore", e.target.id), {
                    onSuccess: () => {
                        Swal.fire({
                                title: 'برگردانده شد!',
    ...
    function handleSearch(e) {
        e.preventDefault()
        get(route("admin.diseases.index"))
    }
    ...
    return (
        <Authenticated
            auth={props.auth}
            errors={props.errors}
            header={<h2 className="font-semibold text-xl leading-tight">بیماری ها</h2>}
        >
                    ...
                    <div className="bg-white overflow-hidden shadow-sm ">
                        <div className="p-6 bg-white dark:bg-gray-800 dark:shadow-xl">
                            ...
                            <table className="w-full text-right text-sm text-left text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                {diseases.data.map(({id, name, deleted_at}) => (
                                    <tr key={id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td className="px-6 py-4">
                                            <Link
                                                ...
                                                href={route("admin.diseases.edit", id)}
                                            >
                                                ویرایش
                                            </Link>
                                            <button
                            ...
                            <Pagination links={diseases.links} class="mt-5"/>
                        </div>
                    </div>
                </div>
            </div>
